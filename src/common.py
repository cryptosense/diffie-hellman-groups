...
def check_duplicates(groups):
    ...
    for group in groups:
        duplicates[group.p].append(group)
    for (p, dup_groups) in duplicates.items():
        if len(dup_groups) < 2:
            continue
        print('Duplicate groups: {}'.format(dup_groups), file=sys.stderr)
...
groups = (
    ...
    # from http://git.haproxy.org/?p=haproxy.git;f=src/ssl_sock.c;hb=d3a341a9
    Group(
        name='HAProxy 1024-bit',
        g=int_from_c_array('0x02,'),
        p=int_from_c_array('''
    ...
    # from http://git.haproxy.org/?p=haproxy.git;f=src/ssl_sock.c;hb=6e774b45
    Group(
        name='HAProxy 1024-bit',
        g=int_from_c_array('0x02,'),
        p=int_from_c_array('''
